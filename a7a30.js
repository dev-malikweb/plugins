$(document).ready(function() {
            ...
            success: function(data) {
                try {
                    ...
                    if (searchElement.length) {
                        ...
                        // 4. جلب الدوال داخل <script id='plugin'>
                        var scriptElement = tempDiv.find("script#plugin");
                        if (scriptElement.length) {
                            var scriptContent = scriptElement.html();
                            var newScript = document.createElement('script');
                            newScript.textContent = scriptContent;
                            document.body.appendChild(newScript); // إضافة السكربت بشكل ديناميكي
                            console.log('تم تنفيذ الكود بنجاح.');
                        } else {
                            console.log('الكود المطلوب غير موجود.');
                        }

                        // الانتقال إلى الخطوة 5 بعد إضافة العنصر والدوال
    ...
    // 5. إعادة تهيئة الدوال بعد إضافة العنصر
